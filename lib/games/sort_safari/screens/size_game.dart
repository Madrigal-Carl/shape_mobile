...
class SizeGameScreen extends StatefulWidget {
  final int lessonId;
  final int studentId;
  final int gameId;

  const SizeGameScreen({
    super.key,
  ...
  @override
  State<SizeGameScreen> createState() => _SizeGameScreenState();
}

class _SizeGameScreenState extends State<SizeGameScreen>
    with TickerProviderStateMixin {
  final Map<String, List<String>> sizeAssets = {
    'small': [
      'small1.png',
      'small2.png',
      'small3.png',
  ...
  late List<String> remainingSizes;
  late String targetSizeKey;
  ...
  void initState() {
    ...
    remainingSizes = sizeAssets.keys.toList()..shuffle();
    _setupNextRound();
  }
  ...
  void _setupNextRound() {
    ...
    setState(() {
      ...
      targetSizeKey = remainingSizes.removeAt(0);
      targetDisplayName = targetSizeKey == 'small' ? 'SMALL' : 'BIG';

      // ✅ Create 12 items with mixed small/big
      final List<String> allAssets = [];
      sizeAssets.forEach((key, assets) => allAssets.addAll(assets));
      allAssets.shuffle();

      final targetAssets = List<String>.from(sizeAssets[targetSizeKey]!);
      targetAssets.shuffle();

      // ✅ 3 correct target items
      ...
      gridItems = items.map((asset) {
        final sizeKey = _inferSizeKeyFromAsset(asset);
        return _GridItem(asset: asset, sizeKey: sizeKey, selected: false);
      }).toList();
    });
  }

  String _inferSizeKeyFromAsset(String asset) {
    final name = asset.toLowerCase();
    if (name.contains('small')) return 'small';
    if (name.contains('big')) return 'big';
    return 'unknown';
  }
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          popupIcon("restart.png", () {
                            setState(() {
                              score = 0;
                              remainingSizes = sizeAssets.keys.toList()
                                ..shuffle();
                              showWinPopup = false;
                              _setupNextRound();
                            });
                          ...
                          popupIcon("next.png", () {
                            Navigator.pushReplacement(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ShapeGameScreen(
                                  lessonId: widget.lessonId,
                                  studentId: widget.studentId,
                                  gameId: widget.gameId,
                                ),
...
class _GridItem {
  ...
  _GridItem copyWith({String? asset, String? sizeKey, bool? selected}) {
    return _GridItem(
